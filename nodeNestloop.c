...
static int popBestPageXid(NestLoopState *node) {
	...
	for (i = 0; i < node->activeRelationPages; i++) {
		node->rewards[i] = node->rewards[i] + 1;
	}
	
    for (i = 0; i < node->activeRelationPages; i++) {
        rewardsTotal = rewardsTotal + node->rewards[i];
    }
	...
	min = 0;
    diff = max - min;
    randomNum = (int) (((double)(diff + 1) / RAND_MAX ) * rand() + min);

	bestPageIndex = 0;
    for (i = 0; i < node->activeRelationPages; i++) {
		if ( (i = 0) ){
			remainder = (randomNum - (node->rewards[i]));
		}
		else {
			remainder = (remainder - (node->rewards[i]));
		}
		if ( (remainder <= 0) ) {
			bestPageIndex = i;
			break;
...
static int popBestPage(NestLoopState *node) {
	...
	for (i = 0; i < node->activeRelationPages; i++) {
		node->tidRewards[i].reward = node->tidRewards[i].reward + 1;
	}
	
    for (i = 0; i < node->activeRelationPages; i++) {
        rewardsTotal = rewardsTotal + node->tidRewards[i].reward;
    }
    ...
    for (i = 0; i < node->activeRelationPages; i++) {
		if ( (i == 0) ){
			remainder = (randomNum - (node->tidRewards[i].reward));
		}
		else {
			remainder = (remainder - (node->tidRewards[i].reward));
		}
		if ( (remainder <= 0) ) {
			bestPageIndex = i;
			break;
